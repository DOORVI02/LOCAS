...
class ReportService:
        ...
        return [
            {
                ...
                "potential_fine": float(t.calculate_days_overdue() * self.config.fine_rate_per_day),
            }
            for t in overdue
        ]
    ...
    def get_active_loans_report(self, limit: int = 100) -> list[dict[str, Any]]:
        ...
        active = self.trans_repo.find_all_with_details(limit=limit, status=TransactionStatus.ACTIVE)
    ...
    def get_system_overview(self) -> dict[str, Any]:
        ...
        return {
            "users": {
                ...
                "students": self.user_repo.count_by_role(student_role.role_id)
                if student_role
                else 0,
                "librarians": self.user_repo.count_by_role(librarian_role.role_id)
                if librarian_role
                else 0,
            },
            "books": {
