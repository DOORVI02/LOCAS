...
from locas.core.exceptions import (
    NotFoundError, ValidationError, AuthorizationError, DuplicateError
)
from locas.core.constants import AuditAction, UserRole
from locas.models.user import User, UserCreate, UserUpdate, Role
...
from locas.utils.validators import validate_email, validate_username, validate_full_name
    ...
    def delete_user(self, user_id: int, force: bool = False) -> bool:
        ...
        if has_deps:
            if not force:
                ...
                if borrowing_count > 0: msg += f"- {borrowing_count} transactions (borrower)\n"
                if issued_count > 0: msg += f"- {issued_count} transactions (issuer)\n"
                if fine_count > 0: msg += f"- {fine_count} fines\n"
