...
from locas.core.exceptions import NotFoundError, AuthorizationError, BusinessRuleError
...
class FineService:
    ...
    def get_fine_by_transaction(self, transaction_id: int) -> Optional[Fine]:
    ...
    def list_fines(
        ...
        Returns:
            List of Fine instances.
        """
        return self.fine_repo.find_all_with_details(
            limit=limit,
            offset=offset,
            status=status
        )
    ...
    def pay_fine(self, fine_id: int) -> Fine:
        ...
        if fine.status != FineStatus.PENDING:
            raise BusinessRuleError(
                f"Cannot pay fine with status '{fine.status.display_name}'"
            )
    ...
    def waive_fine(self, fine_id: int, reason: Optional[str] = None) -> Fine:
        """Waive a fine.
        ...
        if fine.status != FineStatus.PENDING:
            raise BusinessRuleError(
                f"Cannot waive fine with status '{fine.status.display_name}'"
            )
