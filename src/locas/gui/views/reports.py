...
from locas.utils.formatters import format_currency
...
class ReportsView(QWidget):
    ...
    def _load_data(self) -> None:
        ...
        try:
            ...
            self.stat_cards["total_books"].findChild(QLabel, "value").setText(
                str(stats.get("total_books", 0))
            )
            self.stat_cards["total_copies"].findChild(QLabel, "value").setText(
                str(stats.get("total_copies", 0))
            )
            self.stat_cards["active_loans"].findChild(QLabel, "value").setText(
                str(stats.get("active_loans", 0))
            )
            self.stat_cards["overdue_books"].findChild(QLabel, "value").setText(
                str(stats.get("overdue_count", 0))
            )
            self.stat_cards["total_users"].findChild(QLabel, "value").setText(
            ...
            # Fines summary
            self.total_pending.setText(
                f"Total Pending: {format_currency(stats.get('total_pending_fines', 0))}"
            )
            self.total_collected.setText(
                f"Total Collected: {format_currency(stats.get('total_collected_fines', 0))}"
            )
