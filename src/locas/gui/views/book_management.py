...
class BookManagementView(QWidget):
    ...
    def _setup_ui(self) -> None:
        ...
        # Copies tab
        self.copies_panel = CopyManagementPanel(self.config, self.db_manager, self.session_manager)
        ...
        if dialog.exec():
            ...
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Book '{book.title}' created successfully!"
                )
                self._load_data()
    ...
    def _on_edit_book(self, row_data: dict | None = None) -> None:
        ...
        dialog = BookDialog(self.book_service, book=self._selected_book, parent=self)
        if dialog.exec():
            data = dialog.get_data()
            try:
                ...
                book = self.book_service.update_book(self._selected_book.book_id, update_data)
                self._selected_book = book
                self._update_detail_panel()
                self._load_data()

                QMessageBox.information(
                    self, "Success", f"Book '{book.title}' updated successfully!"
                )
                self.data_changed.emit()
